...
def get_ltaf_trendgraph(ltaf_release_name, ltaf_component):
    ...
    report_start_time = local_time + timedelta(days = -60)
    #week = 'week'+time.strftime('%y',time.localtime())+'{:0>2s}'.format(str(z.isocalendar()[1]))
    report_end_date = 'week'+time.strftime('%y',time.localtime())+'{:0>2s}'.format(str(report_end_time.isocalendar()[1]))
    report_start_date = 'week'+time.strftime('%y',time.localtime())+'{:0>2s}'.format(str(report_start_time.isocalendar()[1]))
    ...
    #ltaf_trend_html_link = 'http://pek-lpgtest3.wrs.com/ltaf/nightly_interface.php?release_name=%s&f_type=chart&start_date=%s&end_date=%s&component=%s&tags=IPERF' % (ltaf_release_name, 'week1920', 'week1928', ltaf_component)
...
def summary (fd, productName, spin_name, ltaf_release_name, ltaf_component, matrix_page,
    ...
    msg2html(fd, '<font face="arial" size="6"> %s Weekly Test On Spin: %s </font><br><br>' % (productName, spin_name.split('/')[-1]))
...
def send_nighlty_test_report_email_ltaf(email_to_list, spin_name, ltaf_release_name, ltaf_component, total_time, run_date):
    ...
    testresult_html_link = 'http://pek-lpgtest3.wrs.com/ltaf/nightly_interface.php?release_name=%s&f_type=info&date=%s&component=%s&tags=IPERF' % (ltaf_release_name, week, ltaf_component)
    ...
    ltaf_link='http://pek-lpgtest3.wrs.com/ltaf/nightly_results.php?releasename=%s&clearfilter=true&tf_tr_whentostart=%s&tf_test_component=%s&tf_tr_tags=IPERF' % (ltaf_release_name, week, ltaf_component)
    ...
    mail_title = 'VxWorks 7 Networking Weekly Testing Report (%s) - IPERF' % run_date
...
if __name__ == '__main__':
    ...
    email_to_list = 'haixiao.yan@windriver.com; peng.bi@windriver.com; chunyan.ye@windriver.com; Dong.Liu@windriver.com'
