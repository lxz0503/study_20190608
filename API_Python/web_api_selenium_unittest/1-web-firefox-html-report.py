...
class WebTest(unittest.TestCase):
    ...
    def test_visitURL(self):
        ...
        print(title)
        assert self.driver.title.find(u"????") >= 0, "assert error"

    def test_visitRecentURL(self):
        first_url = "https://www.sogou.com"
    ...
    #     url = "https://www.baidu.com"
    #     self.driver.get(url)
    #     ele = self.driver.find_element_by_xpath('//a[text()="??"]')
    ...
    #         print("the selected option is %s" % option.text)
    #     time.sleep(5)
    #     select_element.deselect_by_visible_text('??')
    ...
    #     berryRadio.click()
    #     time.sleep(5)
    #     self.assertTrue(berryRadio.is_selected(), u"?????????")
    ...
    #     time.sleep(5)
    #     self.assertTrue(berry_check_box.is_selected(), u"?????????")
...
if __name__ == '__main__':
    ...
    file_name = "F:\\xiaozhan_git\\study_20190608\\API_Python\\web_api_selenium_unittest\\report.html"
    with open(file_name, 'wb') as fp:
        runner = HTMLTestRunner(stream=fp,title='report',description='web api test')
        runner.run(suite)
