...
# you can use names to set the name of every column, otherwise you use the default header
df = pd.read_csv('bug_record.csv', header=0, names=['a', 'b', 'c', 'd'])   # read first line, with header
print(df.head(3))
...
df = pd.read_csv('bug_record_no_header.csv', header=0, names=['a', 'b', 'c', 'd'])   # read first line, set header names
print(df.head(3))
