...
def read_plan(plan):
	...
	mode = ws.cell_value(32,0).split('=')[1].lower()
...
def main():
	...
	if os.path.dirname(dvd).endswith('hyan1'):
		command = 'runwassp -f {WASSP_PLAN} -E "WASSP_HOME={WASSP_HOME}" -E "WASSP_WORKSPACE_HOME={WASSP_WORKSPACE_HOME}" -E "WASSP_LOGS_HOME={WASSP_LOGS_HOME}" -E "WASSP_VXWORKS_INSTALL_BASE={WASSP_WIND_HOME}" -E "WASSP_VXWORKS_VERNUM=7" -E "WASSP_VXWORKS_VER=helix" -E "WASSP_VXWORKS_TYPE=vxworks" --continueIfReleaseInvalid --exec-retries 5'.format(WASSP_PLAN = wassp_plan, WASSP_WIND_HOME = dvd, WASSP_HOME = wassp_home, WASSP_WORKSPACE_HOME = workspace, WASSP_LOGS_HOME = logs)
		print('============= {}'.format(command))
	# run on spin
	else:
		command = 'runwassp -f {WASSP_PLAN} -E "WASSP_WIND_HOME={WASSP_WIND_HOME}"  -E "WASSP_HOME={WASSP_HOME}" -E "WASSP_WORKSPACE_HOME={WASSP_WORKSPACE_HOME}" -E "WASSP_LOGS_HOME={WASSP_LOGS_HOME}" --continueIfReleaseInvalid --exec-retries 5'.format(WASSP_PLAN = wassp_plan, WASSP_WIND_HOME = dvd, WASSP_HOME = wassp_home, WASSP_WORKSPACE_HOME = workspace, WASSP_LOGS_HOME = logs)
		print('============= {}'.format(command))
	# run wassp
	try:
		os.system(command)
