...
import os
import sys
sys.path.append('/folk/hyan1/')      # some scripts under this folder,so add this path
from datetime import datetime
import argparse
import xlrd
from itertools import compress
from Nightly.common.connect import Connect    # the dir is like /Nightly/common/connect.py
...
def read_plan(plan):     # this is to read a excel file with xls appendix
	list_case = []
	list_flag = []
	...
	cells_case_name = ws.col_slice(colx=2,     # the second column,start from 0 1 2 3
			          		 	start_rowx=1,   # the first line,which starts from 0 1 2 3
			          		 	end_rowx=5)
...
def main():
	...
	case_name, base, criteria= get_baseline(wassp_plan)
	...
	if float(dict_data[case_name]) / float(base) < float(criteria):
    	# fail
