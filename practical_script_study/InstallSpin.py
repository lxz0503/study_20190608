...
def GetSpinInfo(commit, action):
    ...
    elif commit == 'HELIXSPIN':
        configFile = 'helix_nightly_spin.config'
    elif commit == 'HELIXCISPIN':
        configFile = 'helix_ci_nightly_spin.config'
    elif commit == '653SPIN':
    ...
    if action == 'name':
        ...
        if tsStr:
            ...
            return ''
    elif action == 'lastspin':
        return __GetNameValue('lastS', configFile)
    else:
...
def __GetNameValue(name, configFile):
    debug = False
    if debug:
        configDir = '/net/pek-vx-nwk1/buildarea1/svc-cmnet/log'
    else:
