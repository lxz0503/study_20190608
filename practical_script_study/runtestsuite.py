...
#01sep20,clb support vxsim 2 network interfaces (V7TST-1865)
#24jul20,yyl change behavior when starting RTP for V7COR-7442, V7TST-1848
    ...
    def runTestSuite(self, targets):
        ...
        try:
            for i in range(loops):
                ...
                if self.p['name'] == 'RTNET_RTP' and self.targets:
                ...
                if manual:
                    ...
                    test_cmd = './iptestengine.py ' + " ".join(iptestopts)
...
class vxworks_target(build):
    ...
    def start(self, num, force_bridge):
        ...
        if self.vx['board'] in simicsTargets:
            ...
            if self.p['qemu']:
                return self.startQemuTarget(num, force_bridge)
            else:
...
#            sudovoid(self.globalwrenv + 'wtxregd.ex -d /tmp/&')
        ...
        for i in xrange(num):
            ...
            try:
                if self.p['name'] == 'RTNET':
                    ...
                    cmd = 'cmd rtp exec -v 1 -i -e RTNET_FILE=/romfs/%srtp.json /romfs/%s' % (target_id, os.path.basename(self.p['rtpvxe'][0]))
            ...
            self.host.target_addresses.append(address)
            self.host.targets.append(vlm_target)            
    ...
    def startQemuTarget(self, num, force_bridge):
        # only support itl_generic with 1 interface so far
        from target import ip, QemuTarget, VxWorksBuild, NetworkBuilder, VLMException
        ...
        for i in xrange(num):
            board_name = self.p.get('vxworks').keys()[0]
            binary_name = '%s/%s_vip/default/vxWorks' % (os.getcwd(), self.vx['board'])
            imagePath = binary_name
            if vlmTargets[board_name]['dtb'] is not None:
                dtbPath = '%s/%s' % (os.path.dirname(imagePath), vlmTargets[board_name]['dtb'])
            else:
                dtbPath = None
            vxBuild = VxWorksBuild(imagePath, dtb=None, user='ftp', passwd='interpeak')
            address, _ = self.host.createHostAddress(net_unique_id)
            builds.append(vxBuild)
            ips.append(ip('%s/24' % address))
            targetName = 'QEMU_Q35_%s' % (i + 1)  # must match with qemuTargetConfig.yaml
            qemuTarget = QemuTarget(targetName, logFile='%s.log' % targetName, debug=False)
            targets.append(qemuTarget)
            
            if self.p['name'] == 'RTNET_RTP':
                # RTNet rtp needs the target console for iptestengine.py
                host_type = 'rtnet'
                port = 2000 + int(qemuTarget.Terminal_Server_Port)
                address = qemuTarget.Terminal_Server
            elif self.p['name'] == 'RTNET':
                host_type = 'rtnet'
                port = 23
            else:
        ...
        if force_bridge:
            netbdr = NetworkBuilder(targets, builds, ips, bridge=self.host.umlswitch[0]['name'])
        else:
            netbdr = NetworkBuilder(targets, builds, ips)
        netbdr.setup()
        consoles = netbdr.start()
        self.__waitTargetReady(self.host, timeout=90, interval=1)
        self.__disableCheckSum(targets)
        self.host.targets.append(netbdr)    # cleanup for network bridge and related targets
            
        return 0, False                

    def __disableCheckSum(self, targets):
        print('disable checksum for Qemu targets')
        for target in targets:
            content = '\n'.join(target.sendAndReturnAll('cmd ifconfig -a', '-> ')[1:])
            nicNames = self.__getNICName(content)
            console = target.console
            for n in nicNames:
                console.sendline('cmd ethtool -K %s rx off' % n)
                console.expect('-> ')
                console.sendline('cmd ethtool -K %s tx off' % n)
                console.expect('-> ')                

    def __getNICName(self, content):
        nicNames = []
        lines = content.split('\n')
        for line in lines:
            if not line[0] in (' ', '\t',) and line.find('Link type:') != -1:
                name = line.split(' ')[0].split('\t')[0]
                if name.strip() and name != 'lo0':
        ...
        if opt in ('--vxworks',):
            ...
            if vxconfig['board'] in simicsTargets:
                ...
                if not p['qemu']:
                    ...
                    verifyConfig(vxconfig, vxconfig['board'], ['target', 'imagepath'])
                keyConfig(vxconfig, ['debug', 'ftp'])
                if vxconfig['debug']:
                    noneConfig(vxconfig['debug'], ['mode', 'device', 'address', 'num_tty'])
                if vxconfig['ftp']:
...
# disable line 5068 and 5069 for QEMU
#5061    /*
#5062     * All adapters support VLAN tag insertion and stripping except 82541ER.
#5063     */
#5064    if (pDrvCtrl->geiDevId != INTEL_DEVICEID_82541ER)
#5065    {
#5066    pDrvCtrl->geiCaps.csum_flags_tx |= CSUM_VLAN;
#5067    pDrvCtrl->geiCaps.csum_flags_rx |= CSUM_VLAN;
#5068    pDrvCtrl->geiCaps.cap_available |= IFCAP_VLAN_HWTAGGING;
#5069    pDrvCtrl->geiCaps.cap_enabled |= IFCAP_VLAN_HWTAGGING;
#5070    }

def disableVlanHwTagging(installPath):
    sourceFile = 'vxbGei825xxEnd.c'
    print('disable VLAN HW tagging at %s for QEMU build' % sourceFile)
    content, newLines = '', []
    
    filePath = findFile(installPath, sourceFile)
    if not filePath:
        raise Exception('%s not found under %s' % (sourceFile, installPath))
    
    with open(filePath, 'rt') as fdr:
        content = fdr.read()
        
    ptn = '(?s)All adapters support VLAN tag insertion and stripping except 82541ER\..*?if \(pDrvCtrl->geiDevId != INTEL_DEVICEID_82541ER\).*?\{(.*?)\}'
    found = re.search(ptn, content)
    if found is not None:
        old = found.groups()[0]
        for line in old.split('\n'):
            if line.find('IFCAP_VLAN_HWTAGGING') != -1 and line.find('/*') == -1 and line.find('*/') == -1:
                newLines.append('/* ' + line + ' */')
            else:
                newLines.append(line)
        with open(filePath, 'wt') as fdw:
            fdw.write(content.replace(old, '\n'.join(newLines)))
    else:
...
def testOneProduct(argv, log, ntest):
    ...
    try:
        try:
            p, bopts = parseArguments(argv, ntest)

            if debugFlag:
                import pprint as pp
                print '=== p=', pp.pprint(p)

            board = p['vxworks'].keys()[0]
                ...
                for target in targets:
                    if isinstance(target, vxworks_target):
                        ...
                        if target.p['rebuild']:
                            ...
                            if p['qemu'] and target.vx['board'] in vlmTargets:
                                disableVlanHwTagging(target.vx['path'])
            ...
            if p['qemu']:
                force_bridge = p['codecoverage'] or p['use_extdev'] or p['use_umldev']
            else:
